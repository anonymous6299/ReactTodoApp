...
const Todo = ({ props }) => {
    ...
    useEffect(() => {
        if (props.item.completed) {
            setBg({color:"#bbf7d0",border:"#14532d"})
        }
        else if(props.item.priority.includes("h")&&(!(props.item.completed))){
            setBg({color:"#FEE2E2",border:"#EF4444"})
        }
        else if(props.item.priority.includes("m")&&(!(props.item.completed))){
            setBg({color:"#FEF3C7",border:"#F59E0B"})
        }
        else if(props.item.priority.includes("l")&&(!(props.item.completed))){
            setBg({color:"#D1FAE5",border:"#10B981"})
        }
        else{
            setBg({color:"#F9FAFB",border:"#c5c5c5"})
        }
    }, [props.item])

    return (
        <>
            <div className="border w-80 rounded-xl px-4 py-4 hover:shadow-lg"
                style={{
